...
def _process():
    ...
    if not data or not KEYS_ROOT["required"].issubset(data.keys()):
        return jsonify(
            {
                "error": f"Invalid or missing JSON payload. Required fields: {list(KEYS_ROOT['required'])}"
            }
        ), 400
    ...
    if missing_params:
        return jsonify(
            {
                "error": f"Missing required parameters for type '{request_type}': {list(missing_params)}"
            }
        ), 400
...
def controller(data: dict, session_id: str):
    try:
        ...
        print(
            f'{status} routeId="{current_request.route_id}" (type="{request_type}", criterion="{criterion}", sessionId="{session_id}")'
        )
