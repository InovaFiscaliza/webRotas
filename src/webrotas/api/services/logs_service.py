...
- Fetch container logs via podman/docker CLI
            ...
            try:
                with open(log_file, 'r', encoding='utf-8') as f:
                    ...
                    for line in reversed(lines):
                        ...
                        # Apply filters
                        if log_level and f"[{log_level}]" not in line and log_level not in line:
                            continue
...
def get_container_logs(
    ...
    """
    Retrieve OSRM container logs using podman/docker CLI.
    ...
    try:
        ...
        for cmd in commands:
            try:
                ...
                # CLI not available, try next one
                continue
            except subprocess.TimeoutExpired:
                logger.warning(f"Timeout fetching logs from {cmd[0]} container {container_name}")
                continue
            except Exception as e:
                logger.debug(f"Error with {cmd[0]}: {e}")
                continue
        
        if logs is None:
            return {
                "status": "warning",
                "message": f"Container '{container_name}' not found or not running. Ensure podman/docker is running.",
        ...
        logger.error(f"Error retrieving container logs: {e}", exc_info=True)
        return {
            "status": "error",
            "message": str(e),
            "container_name": container_name,
...
def get_combined_logs(
    ...
    container_logs = get_container_logs(container_name=container_name, tail=container_tail)
...
def _extract_timestamp(log_line: str) -> Optional[str]:
    ...
    try:
        ...
        if len(log_line) > 19:
            ...
            if (len(potential_timestamp) == 19 and 
                potential_timestamp[4] == '-' and 
                potential_timestamp[7] == '-' and
                potential_timestamp[10] == ' ' and
                potential_timestamp[13] == ':' and
                potential_timestamp[16] == ':'):
