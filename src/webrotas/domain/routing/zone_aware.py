...
def _offset_point(lat: float, lng: float, direction: str, offset_km: float = 1.0) -> Tuple[float, float]:
    """
...
def generate_boundary_waypoints(
    ...
    Args:
        ...
        offset_km: Offset distance in km from zone boundary
    ...
    for zone_idx, (polygon, zone_data) in enumerate(zip(polygons, avoid_zones_list)):
        try:
            ...
            # Generate cardinal direction waypoints
            ...
            logger.info(
                f"Generated 4 boundary waypoints for zone '{zone_data.get('name', f'Zone {zone_idx}')}'"
            )
...
async def try_route_with_intermediate_waypoints(
    ...
    try:
        ...
        logger.info(f"Trying route with {len(intermediate_coords)} intermediate waypoint(s)")
...
async def find_route_around_zones(
    ...
    try:
        ...
        for offset_km in offset_distances:
            ...
            logger.info(f"Trying {len(boundary_points)} boundary waypoints at {offset_km}km offset")
