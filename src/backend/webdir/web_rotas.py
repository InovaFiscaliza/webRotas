...
#-----------------------------------------------------------------------------------#
def osrm_shortest(current_request, session_id, origin, waypoints, avoid_zones, criterion, location_limits=[], location_urban_areas=[]):
...
#-----------------------------------------------------------------------------------#
def osrm_circle(current_request, session_id, origin, center_point, radius_km, total_waypoints, avoid_zones, criterion):
    ## <Função escopo local> ##
    def generate_waypoints_in_radius(center_point, radius_km, total_waypoints):
        ...
        for ii in range(total_waypoints):
            ...
            new_lon_rad = lng_rad + math.atan2(math.sin(angle) * math.sin(radius_km / R) * math.cos(lat_rad), math.cos(radius_km / R) - math.sin(lat_rad) * math.sin(new_lat_rad))

            new_lat = math.degrees(new_lat_rad)
            new_lon = math.degrees(new_lon_rad)
            waypoints.append({"lat": np.round(new_lat, 6), "lng": np.round(new_lon, 6), "description": ""})

        return waypoints
    ## </Função escopo local> ##

    waypoints = generate_waypoints_in_radius(center_point, radius_km, total_waypoints)
    osrm_shortest(current_request, session_id, origin, waypoints, avoid_zones, criterion)

#-----------------------------------------------------------------------------------#
def osrm_grid(current_request, session_id, origin, city, state, scope, point_distance, avoid_zones, criterion):
    ## <Função escopo local> ##
    def generate_waypoints_in_city(city_boundaries: list, avoid_zones: list, point_distance: int) -> list:
        city_polygon_list  = []
        avoid_polygon_list = []
        
        for boundary in city_boundaries:
            city_polygon_list.append(Polygon([(float(lng), float(lat)) for lng, lat in boundary]))
        ...
        for zone in avoid_zones:
            ...
            """
            avoid_polygon_list.append(Polygon([(float(lng), float(lat)) for lat, lng in zone["coord"]]))
        ...
        for lat in lat_range:
            for lng in lng_range:
                point = Point(lng, lat)
                if any(city_polygon.contains(point) for city_polygon in city_polygon_list):
                    inside_avoid = any(zone.contains(point) for zone in avoid_polygon_list)
                    if not inside_avoid:
                        waypoints.append({"lat": np.round(lat, 6), "lng": np.round(lng, 6), "description": ""})

        if not waypoints:
            raise ValueError(f'No waypoints found for scope "{scope}" with a point distance of {point_distance} meters.')
        return waypoints
    ## </Função escopo local> ##
    ...
    match scope:
        case "Location":
            waypoints = generate_waypoints_in_city(location_limits,      avoid_zones, point_distance)
        case "UrbanAreas":
            waypoints = generate_waypoints_in_city(location_urban_areas, avoid_zones, point_distance)
        case _:
            raise ValueError(f'Invalid scope "{scope}"')
        
    osrm_shortest(current_request, session_id, origin, waypoints, avoid_zones, criterion, location_limits, location_urban_areas)

#-----------------------------------------------------------------------------------#
def osrm_ordered(current_request, session_id, origin, cache_id, bounding_box, waypoints, avoid_zones, criterion):
    # routing_area, bounding_box, cache_id = compute_routing_area(avoid_zones, origin, waypoints)
    # si.PreparaServidorRoteamento(routing_area)
...
def compute_routing_area(avoid_zones, origin, waypoints):
    ...
    def compute_bounding_box(origin, waypoints, padding_km=50):
        ...
        return round(lat_min, 6), round(lat_max, 6), round(lng_min, 6), round(lng_max, 6)
    ## </Função escopo local> ##
...
def get_areas_urbanas_cache(city, state):
    ...
    if not cache_polylines:
        ...
        cache_polylines = [f"{city}-{state}", polMunicipio, polMunicipioAreasUrbanizadas]
