...
def AtivaServidorWebRotas():
    ...
    if not teste:
        if log_filename == "":
            ...
            wLog(f"Ativando Servidor log {log_filename}")
...
def PreparaServidorRoteamento(regioes):
    ...
    GeraArquivoExclusoes(
        regioes,
        arquivo_saida=f"../../resources/Osmosis/TempData/exclusion_{UserData.nome}.poly",
    )
    if not VerificaArquivosIguais(
        f"../../resources/Osmosis/TempData/exclusion_{UserData.nome}.poly",
        f"../../resources/Osmosis/TempData/exclusion_{UserData.nome}.poly.old",
    ):
